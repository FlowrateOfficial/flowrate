...
<template>
  <DashboardPageShell :show-guide="false">
    ...
    >
      <UCard :ui="{ body: 'p-4 sm:p-5' }">
        <h2 class="mb-3 text-base font-semibold">
          {{ t('dashboard.family.inviteGuardian') }}
        </h2>
        <div class="grid sm:grid-cols-2 gap-4">
          <UFormField :label="t('dashboard.family.emailPlaceholder')">
            <UInput
              v-model="familyStore.inviteForm.email"
              type="email"
              :placeholder="t('dashboard.family.emailPlaceholder')"
              class="w-full"
            />
          </UFormField>
          <UFormField :label="t('dashboard.family.namePlaceholder')">
            <UInput
              v-model="familyStore.inviteForm.name"
              :placeholder="t('dashboard.family.namePlaceholder')"
              class="w-full"
            />
          </UFormField>
        </div>
      ...
      <UCard :ui="{ body: 'p-0 sm:p-0' }">
        ...
        >
          ...
          <template #role-cell="{ row }">
            <UBadge
              :label="familyStore.roleLabel(row.original.role)"
              variant="subtle"
              color="neutral"
              size="xs"
            />
          ...
          <template #status-cell="{ row }">
            <UBadge
              :label="familyStore.statusLabel(row.original.status)"
              variant="subtle"
              size="xs"
            />
          </template>
    ...
    >
      <UCard :ui="{ body: 'p-4 sm:p-5' }">
        <h2 class="mb-1 text-base font-semibold">
          {{ t('dashboard.family.addChildTitle') }}
        </h2>
        <p class="mb-4 text-sm text-muted">
          {{ t('dashboard.family.addChildDescription') }}
        </p>
        <div class="grid sm:grid-cols-2 gap-4">
          <UFormField :label="t('dashboard.family.username')">
            <UInput
              v-model="familyStore.childForm.username"
              :placeholder="t('dashboard.family.usernamePlaceholder')"
              class="w-full"
            />
          </UFormField>
          <UFormField :label="t('dashboard.family.childEmail')">
            <UInput
              v-model="familyStore.childForm.email"
              type="email"
              :placeholder="t('dashboard.family.childEmailPlaceholder')"
              class="w-full"
            />
            <template #help>
              {{ t('dashboard.family.childEmailHelp') }}
            </template>
          </UFormField>
          <UFormField :label="t('dashboard.family.password')">
            <UInput
              v-model="familyStore.childForm.password"
              type="password"
          ...
          <UFormField :label="t('dashboard.family.confirmPassword')">
            <UInput
              v-model="familyStore.childForm.confirmPassword"
              type="password"
              autocomplete="new-password"
              class="w-full"
            />
          ...
          <UFormField :label="t('dashboard.family.birthday')">
            <DashboardDateField
              v-model="familyStore.childForm.birthday"
              :placeholder="t('dashboard.family.birthday')"
            />
          </UFormField>
          <UFormField :label="t('dashboard.family.roleLabel')">
            <USelect
              v-model="familyStore.childForm.role"
              :items="familyStore.childRoleItems"
              class="w-full"
            />
          </UFormField>
        </div>
        <UButton
          class="mt-4"
          :label="t('dashboard.family.createAccount')"
          icon="i-lucide-user-plus"
          :loading="inviting"
          @click="familyStore.addChild(spaceId)"
        />
      </UCard>
      ...
      <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        >
          <div class="flex items-start justify-between gap-3 mb-4">
            ...
            <UBadge
              :label="familyStore.statusLabel(child.status)"
              variant="subtle"
              size="xs"
            />
          </div>
          ...
          <div
            v-if="child.financialSummary"
            class="mb-3 grid grid-cols-2 gap-2"
          >
            <div class="rounded-lg bg-elevated/50 p-2.5">
              <p class="text-xs text-muted">
                {{ t('dashboard.family.childBalance') }}
              </p>
              <p class="text-base font-semibold tabular-nums">
                {{ familyStore.fmt(child.financialSummary.balance) }}
              </p>
            </div>
            <div class="rounded-lg bg-elevated/50 p-2.5">
              <p class="text-xs text-muted">
                {{ t('dashboard.family.spending30d') }}
              </p>
              <p class="text-base font-semibold tabular-nums">
                {{ familyStore.fmt(child.financialSummary.spending30d) }}
              </p>
            </div>
          </div>

          <div
            v-if="child.childProfile"
    ...
    >
      <UCard :ui="{ body: 'p-4 sm:p-5' }">
        <h2 class="mb-3 text-base font-semibold">
          {{ t('dashboard.family.splits.addTitle') }}
        </h2>
        <div class="grid sm:grid-cols-2 gap-4">
          <UInput
            v-model="familyStore.splitForm.name"
            :placeholder="t('dashboard.family.splits.namePlaceholder')"
          />
          <USelect
        ...
        <UButton
          class="mt-4"
          :label="t('common.add')"
          @click="familyStore.createSplit(spaceId)"
        />
      </UCard>
      <UEmpty
      ...
      >
        <UCard
          v-for="rule in familyStore.splits"
          :key="rule.id"
          :ui="{ body: 'p-3 sm:p-4' }"
        >
          <p class="font-medium">
          ...
          <p class="text-xs text-muted mt-1">
            {{ rule.mode }} · {{ rule.category ?? t('dashboard.family.splits.allCategories') }}
          </p>
        </UCard>
      </div>
    </div>
